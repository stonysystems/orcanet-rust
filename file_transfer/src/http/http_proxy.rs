...
//     async fn serve(&mut self, io: TokioIo<TcpStream>) {
...
// impl ProxyRequestHandler for ProxyClient {
//     async fn serve(&mut self, io: TokioIo<TcpStream>) {
//         let config_clone = self.config.clone();
//
//         tokio::task::spawn(async move {
//             if let Err(err) = http1::Builder::new()
//                 .serve_connection(io, service_fn(|req| Self::handle_request(req, config_clone)))
//                 .await {
//                 tracing::error!("Error serving connection: {:?}", err);
//             }
//         });
