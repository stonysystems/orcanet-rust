...
async fn handle_provide_request(
...
async fn handle_request(request: Request<Incoming>,
                        mode: ProxyMode)
                        -> Result<Response<Full<Bytes>>, Error> {
    match mode {
        ProxyMode::ProxyClient(config) => {
            handle_use_request(request, config)
                .await
        },
        ProxyMode::ProxyProvider => {
            handle_provide_request(request)
                .await
        }
    }
}
            ...
            stream_event = listener.accept() => {
                ...
                tokio::task::spawn(async move {
                    if let Err(err) = http1::Builder::new()
                        ...
                        let mode_clone_inner = mode_clone.clone();  // Clone for each request
                        async move {
                            handle_request(req, mode_clone_inner).await
                        }
                    }))
                        .await {
